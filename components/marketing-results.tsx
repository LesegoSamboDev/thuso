...
import { Target, Banknote, TrendingUp, Video, Zap, Wrench, Users, MessageSquare, ChartBar as BarChart2 } from 'lucide-react';
...
function ResultSection({ icon, title, accent, children }: ResultSectionProps) {
  return (
    <div className="rounded-2xl glass-card p-6 border border-slate-200/90 hover:border-slate-300 dark:border-white/[0.07] dark:hover:border-white/[0.12] transition-all duration-300">
...
function Pill({ children }: { children: React.ReactNode }) {
  return (
    <span className="inline-flex items-center px-3 py-1 rounded-lg bg-slate-100 border border-slate-200 text-xs text-slate-700 dark:bg-white/[0.05] dark:border-white/[0.08] dark:text-slate-300 leading-relaxed">
      {children}
    </span>
  );
}
      ...
      >
        <div className="space-y-4">
          ...
          <div>
            ...
            <p className="text-sm text-slate-700 dark:text-slate-300 leading-relaxed">
              {strategy.positioning.competitiveAdvantage}
            </p>
          </div>
          <div>
            <p className="text-xs text-slate-500 font-medium mb-1.5">Brand Voice</p>
            <p className="text-sm text-slate-700 dark:text-slate-300 leading-relaxed">
              {strategy.positioning.brandVoice}
            </p>
          </div>
        </div>
      </ResultSection>
      ...
      >
        <div className="space-y-4">
          <div className="flex items-start gap-3 p-3 rounded-xl bg-amber-500/5 border border-amber-500/10">
            ...
            <p className="text-sm text-amber-900/90 dark:text-amber-200/80 leading-relaxed font-medium">
              {strategy.pricingFeedback.assessment}
            </p>
          </div>
          <div>
            <p className="text-xs text-slate-500 font-medium mb-1.5">Recommendation</p>
            <p className="text-sm text-slate-700 dark:text-slate-300 leading-relaxed">
              {strategy.pricingFeedback.recommendation}
            </p>
          </div>
          <div>
            <p className="text-xs text-slate-500 font-medium mb-1.5">Reasoning</p>
            <p className="text-sm text-slate-700 dark:text-slate-300 leading-relaxed">
              {strategy.pricingFeedback.reasoning}
            </p>
          </div>
        </div>
      </ResultSection>
      ...
      >
        <div className="space-y-3">
          {strategy.hooks.map((hook, i) => (
            <div
              key={i}
              className="flex items-start gap-3 p-3 rounded-xl bg-slate-50 border border-slate-200 dark:bg-white/[0.03] dark:border-white/[0.06]"
            >
              <span className="text-xs text-slate-500 dark:text-slate-600 font-mono mt-0.5 w-4 shrink-0">
                {String(i + 1).padStart(2, '0')}
              </span>
              <p className="text-sm text-slate-800 dark:text-slate-200 leading-relaxed italic">&ldquo;{hook}&rdquo;</p>
            </div>
          ))}
        </div>
      </ResultSection>
